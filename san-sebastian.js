{
  ...
  "address": {
    "@type": "PostalAddress",
    "streetAddress": "Recto Ave., Quiapo",
...
(function(){
      var RATES={basketball:{off:1500,day:1800,night:2100},volleyball:{off:1500,day:1800,night:2100},pickleball:{off:600,day:700,night:800}};
      var ADDONS=[
      ...
      var SCRIPT_URL='https://script.google.com/macros/s/AKfycbyYSgQKWvMpZ5FFx7yLQFc_AqtVtQel4coB4_2koaJg4OyerRzdcteTk3xr69hM0otP/exec';
      var CALENDARS={
        fullcourt:   'c_8d94f6689d69d10d202c7a928c7cd840bef0fd7954b26ecbbe5accec62ce7cf4@group.calendar.google.com',
        pickleballA: 'c_a91f149d87d6e6c9d5c5a20e5e399594864742d95442a1690295a40be16874a0@group.calendar.google.com',
      ...
      document.addEventListener('DOMContentLoaded',function(){
        ...
        $('bwFinal').addEventListener('click',async function(){
          ...
          var payload={
            ...
            players:selSport==='pickleball'?selPlayers+' \xd7 ₱100 = ₱'+playerTotal:''
          };
          try{
...
function switchSSPanel(panel) {
  document.getElementById('ssCourtPanel').classList.toggle('active', panel === 'court');
  document.getElementById('ssGymPanel').classList.toggle('active', panel === 'gym');
  document.getElementById('ssToggleCourt').classList.toggle('active', panel === 'court');
  document.getElementById('ssToggleGym').classList.toggle('active', panel === 'gym');
}

function submitSSGymInquiry(e) {
  e.preventDefault();
  const name  = document.getElementById('ssGiName').value;
  const phone = document.getElementById('ssGiPhone').value;
  const email = document.getElementById('ssGiEmail').value;
  const type  = document.getElementById('ssGiType').value;
  const msg   = document.getElementById('ssGiMsg').value;
  const subject = encodeURIComponent('Gym Membership Inquiry – San Sebastian – ' + name);
  const body    = encodeURIComponent('Name: ' + name + '\nPhone: ' + phone + '\nEmail: ' + email + '\nMembership: ' + type + (msg ? '\n\nMessage:\n' + msg : ''));
  window.location.href = 'mailto:sales@fitnessexclusive.ph?subject=' + subject + '&body=' + body;
  document.getElementById('ssGiSuccess').classList.add('show');
  e.target.reset();
}
...
(function(){
  ...
  const track   = document.getElementById('ssCarouselTrack');
  const slides  = track.querySelectorAll('.carousel-slide');
  const dots    = document.querySelectorAll('#ssCarouselDots .carousel-dot');
  const thumbs  = document.querySelectorAll('#ssCarouselThumbs .carousel-thumb');
  const counter = document.getElementById('ssCarouselCounter');
  function goTo(index) {
  ...
  document.getElementById('ssCarouselPrev').addEventListener('click', () => { resetAuto(); goTo(current - 1); });
  document.getElementById('ssCarouselNext').addEventListener('click', () => { resetAuto(); goTo(current + 1); });
  ...
  let startX = 0;
  const wrap = document.getElementById('ssCarousel');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var courtPanel = document.getElementById('ssCourtPanel');
